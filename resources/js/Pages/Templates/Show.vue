...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="py-8 max-w-5xl mx-auto px-4">
            ...
            <!-- Success -->
            <div v-if="success" class="mb-6 p-4 bg-green-50 border border-green-200 rounded-xl text-green-700 font-medium text-center">
            ...
            <div class="bg-white rounded-2xl shadow-md border border-gray-100 p-6 mb-8">
                ...
                <div v-if="children.length === 0" class="text-gray-400 text-sm">
                    Nie masz jeszcze żadnego dziecka. <a :href="route('children.create')" class="text-blue-600 font-medium hover:underline">Dodaj dziecko</a>
                </div>
                <div v-else class="flex flex-col sm:flex-row gap-3">
                    <select
                        v-model="form.child_id"
                        class="flex-1 px-4 py-3 rounded-xl border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none bg-gray-50 text-gray-800"
                    >
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                <div
                    ...
                    :key="item.id"
                    class="bg-white rounded-xl border border-gray-100 shadow-sm p-3 flex flex-col items-center gap-2 hover:shadow-md transition-shadow"
                >
                    <img
                        :src="item.image_path"
                        :alt="item.name"
                    ...
                    <span class="text-xs text-gray-400">{{ item.category_name }}</span>
                </div>
            </div>
        </div>
    </AuthenticatedLayout>
